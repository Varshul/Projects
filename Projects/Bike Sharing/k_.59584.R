...
fit1 <- randomForest(count ~ d  + temp + atemp + t + humidity + holiday + workingday + season + weather + windspeed, data=train, importance=T,ntree=1000)
varImpPlot(fit1)
p <- predict(fit1, test,interval=("confidence"))
s <- data.frame(datetime = test$datetime, count = p)

write.csv(s, file = "submit8.csv", row.names=F)
