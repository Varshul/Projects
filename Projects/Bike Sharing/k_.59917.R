...
fit1 <- randomForest(count ~ t  + d + season + holiday + workingday + weather + temp+ atemp + humidity + windspeed, data=train, importance=T,ntree=1000)
varImpPlot(fit1)
p <- predict(fit1, test,interval=("confidence"))
s <- data.frame(datetime = test$datetime, count = p)

write.csv(s, file = "submit7.csv", row.names=F)
